...
class NativeAPIService {
  ...
  /// This function is used to check whether the local auth is available on device or not
  Future<bool> isDeviceLocalAuthAvailable() async {
    // bool _isLocalAuthAvailable = false;
    // try {
    //   _isLocalAuthAvailable = await Application.localAuth.canCheckBiometrics;
    // } on PlatformException catch (e) {
    //   Application.logService!.log(
    //     '(TRACE) NativeAPIService: isDeviceLocalAuthAvailable: Error',
    //     type: Log.ERROR,
    //     error: e,
    //   );
    // }
    // return _isLocalAuthAvailable;
    return true;
  }
  ...
  Future<List<BiometricType>> getEnrolledBiometrics() async {
    ...
    try {
      ...
      Application.logService!.log(
        '(TRACE) NativeAPIService: getEnrolledBiometrics: Error',
        type: Log.ERROR,
        error: e,
      );
    }
  ...
  Future<bool> biometricAuthenticate() async {
    ...
    try {
      String message;
      if (Application.enrolledBiometrics.contains(BiometricType.face)) {
        message =
            "Please authenticate using face to continue"; // todo: translate error
      } else {
        message =
            "Please scan your fingerprint to authenticate"; // todo: translate error
      }

      // didAuthenticate = await Application.localAuth.authenticate(
      ...
      Application.logService!.log(
        '(TRACE) NativeAPIService: biometricAuthenticate: Error',
        type: Log.ERROR,
        error: e,
      );
    }
  ...
  Future<Map<String, dynamic>> pickImage(ImageSource source) async {
    try {
      ...
      Application.logService!.log("Pick Image", type: Log.ERROR, error: e);
      return _error;
    }
  }
  ...
  //     Application.logService!.log("Original Image Size: ${file.lengthSync()}");
  //     Application.logService!.log(
  //       "Compressed Image Size: ${_compressedFile?.lengthSync()}",
  //     );
