import 'package:bwys/screens/product/model/product_model.dart';
import 'package:flutter/material.dart';

List items = [
  {
    "videoUrl": "assets/videos/video_1.mp4",
    "name": "Vannak Niza🦋",
    "caption": "Morning, everyone!!",
    "songName": "original sound - Łÿ Pîkâ Ćhûû",
    "profileImg":
        "https://p16-tiktokcdn-com.akamaized.net/aweme/720x720/tiktok-obj/1663771856684033.jpeg",
    "likes": "1.5M",
    "comments": "18.9K",
    "shares": "80K",
    "albumImg":
        "https://images.unsplash.com/photo-1502982899975-893c9cf39028?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60"
  },
  {
    "videoUrl": "assets/videos/video_2.mp4",
    "name": "Dara Chamroeun",
    "caption": "Oops 🙊 #fyp #cambodiatiktok",
    "songName": "original sound - 💛💛Khantana 🌟",
    "profileImg":
        "https://p16-tiktokcdn-com.akamaized.net/aweme/720x720/tiktok-obj/ba13e655825553a46b1913705e3a8617.jpeg",
    "likes": "4.4K",
    "comments": "5.2K",
    "shares": "100",
    "albumImg":
        "https://images.unsplash.com/photo-1462804512123-465343c607ee?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=668&q=80"
  },
  {
    "videoUrl": "assets/videos/video_3.mp4",
    "name": "9999womenfashion",
    "caption": "#블루모드",
    "songName": "original sound - 🖤Khün MÄk🇰🇭",
    "profileImg":
        "https://p16-tiktokcdn-com.akamaized.net/aweme/720x720/tiktok-obj/1664576339652610.jpeg",
    "likes": "100K",
    "comments": "10K",
    "shares": "8.5K",
    "albumImg":
        "https://images.unsplash.com/photo-1457732815361-daa98277e9c8?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1650&q=80"
  },
];

List categoryItems = [
  {
    "id": 1,
    "name": "Men",
    "imageURl":
        "https://media.istockphoto.com/photos/positivity-puts-you-in-a-position-of-power-picture-id1299077582?b=1&k=20&m=1299077582&s=170667a&w=0&h=Esjqlg_WCWmTc83Dv6PLhwPFwYN9uXoclBn0cUhtS5I=",
  },
  {
    "id": 2,
    "name": "Women",
    "imageURl":
        "https://images.unsplash.com/photo-1508057198894-247b23fe5ade?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80"
  },
  {
    "id": 3,
    "name": "Kids",
    "imageURl":
        "https://images.unsplash.com/photo-1571210862729-78a52d3779a2?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80"
  },
  {
    "id": 4,
    "name": "Beauty",
    "imageURl":
        "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU="
  },
  {
    "id": 5,
    "name": "Luxury",
    "imageURl":
        "https://media.istockphoto.com/photos/modern-living-room-interior-3d-render-picture-id1293762741?b=1&k=20&m=1293762741&s=170667a&w=0&h=2RI8SmBN4MrEZuTvdwRzaeB887x-dukFcQBpyQ-qwS4="
  },
  {
    "id": 6,
    "name": "Accessories",
    "imageURl":
        "https://media.istockphoto.com/photos/gamer-work-space-concept-top-view-a-gaming-gear-mouse-keyboard-in-picture-id1170073824?b=1&k=20&m=1170073824&s=170667a&w=0&h=E2W21WamGtyTwz03FlSwIaA24uYF3oLIVwZr0uAJCPY="
  }
];

List productItems = [
  {
    "category_id": 1,
    "product_name": "Men",
    "product_id": 21,
    "cost_price": 211,
    "selling_price": 221,
    "is_available": true,
    "total_reviews": 3223,
    "imageURl":
        "https://media.istockphoto.com/photos/positivity-puts-you-in-a-position-of-power-picture-id1299077582?b=1&k=20&m=1299077582&s=170667a&w=0&h=Esjqlg_WCWmTc83Dv6PLhwPFwYN9uXoclBn0cUhtS5I=",
  },
  {
    "category_id": 2,
    "product_name": "Woman",
    "product_id": 212,
    "cost_price": 2112,
    "selling_price": 2212,
    "is_available": true,
    "imageURl":
        "https://images.unsplash.com/photo-1508057198894-247b23fe5ade?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80",
  },
  {
    "category_id": 3,
    "product_name": "Woman",
    "product_id": 212,
    "cost_price": 2112,
    "selling_price": 2212,
    "is_available": true,
    "imageURl":
        "https://images.unsplash.com/photo-1571210862729-78a52d3779a2?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80"
  },
  {
    "category_id": 4,
    "product_name": "Woman",
    "product_id": 212,
    "cost_price": 2112,
    "selling_price": 2212,
    "is_available": true,
    "imageURl":
        "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU="
  },
];

// Our demo Products

List<Product> demoProducts = [
  Product(
    id: 1,
    sizes: [
      ProductSizes(size: "10", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "11", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "12", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "11", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "12", noOfPiece: 2, isSelected: false),
    ],
    totalRating: 134,
    images: [
      "https://images.unsplash.com/photo-1571210862729-78a52d3779a2?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80",
      "https://images.unsplash.com/photo-1571210862729-78a52d3779a2?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80",
      "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU=",
      "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU=",
    ],
    discount: 10.0,
    title: "Pepe Jeans",
    price: 64.99,
    description: description,
    rating: 4.8,
    isFavourite: true,
    isPopular: true,
  ),
  Product(
    id: 2,
    sizes: [
      ProductSizes(size: "8", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "9", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "10", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "11", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "12", noOfPiece: 2, isSelected: false),
    ],
    totalRating: 134,
    images: [
      "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU="
    ],
    discount: 10.0,
    title: "Nike Sport White - Man Pant",
    price: 450.5,
    description: description,
    rating: 4.1,
    isPopular: true,
  ),
  Product(
    id: 3,
    totalRating: 134,
    sizes: [
      ProductSizes(size: "10", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "11", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "12", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "11", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "12", noOfPiece: 2, isSelected: false),
    ],
    images: [
      "https://images.unsplash.com/photo-1571210862729-78a52d3779a2?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80",
      "https://images.unsplash.com/photo-1571210862729-78a52d3779a2?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80",
      "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU=",
      "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU=",
    ],
    discount: 10.0,
    title: "Pepe Jeans",
    price: 764.99,
    description: description,
    rating: 4.8,
    isFavourite: true,
    isPopular: true,
  ),
  Product(
    id: 4,
    totalRating: 134,
    sizes: [
      ProductSizes(size: "7", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "8", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "9", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "10", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "11", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "12", noOfPiece: 2, isSelected: false),
    ],
    images: [
      "https://images.unsplash.com/photo-1508057198894-247b23fe5ade?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=870&q=80"
    ],
    discount: 10.0,
    title: "Gloves XC Omega - Polygon",
    price: 836.55,
    description: description,
    rating: 4.1,
    isFavourite: true,
    isPopular: true,
  ),
  Product(
    id: 5,
    totalRating: 134,
    sizes: [
      ProductSizes(size: "7", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "8", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "9", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "10", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "11", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "12", noOfPiece: 2, isSelected: false),
    ],
    images: [
      "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU="
    ],
    discount: 10.0,
    title: "Logitech Head",
    price: 920.20,
    description: description,
    rating: 4.1,
    isFavourite: true,
  ),
  Product(
    id: 6,
    totalRating: 134,
    sizes: [
      ProductSizes(size: "7", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "8", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "9", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "10", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "11", noOfPiece: 2, isSelected: false),
      ProductSizes(size: "12", noOfPiece: 2, isSelected: false),
    ],
    images: [
      "https://media.istockphoto.com/photos/young-beautiful-woman-picture-id1294339577?b=1&k=20&m=1294339577&s=170667a&w=0&h=_5-SM0Dmhb1fhRdz64lOUJMy8oic51GB_2_IPlhCCnU="
    ],
    discount: 10.0,
    title: "Logitech Head",
    price: 1020.20,
    description: description,
    rating: 4.1,
    isFavourite: true,
  )
];

const String description = "Wireless Controller for PS4™ gives you what you ";
